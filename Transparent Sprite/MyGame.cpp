#include "MyDirectX.h"
#include<sstream>
const string APPTITLE = "Tile_Based Dynamic Scrolling";
...
//this function does the real worl of drawing a single tile from the
...
//This function files the tilebuffer with tiles representing
...
void DrawTiles()
{
	...
	for(y = 0; y <= rows; ++y)
		for (x = 0; x <= columns; ++x)
		{
			//retrieve the tile number from this posision
...
//buffer according to the current "partial tile" scroll posiotion.
void DrawScrollWindow(bool scaled = false)
{
	...
	if (scaled)
	{
		//used this line for scaled display
		RECT r = { 0, 0, WINDOWWIDTH - 1, WINDOWHEIGHT - 1 };
		r2 = r;
	}
	else
	{
		//used this line foe none-scalede display
		RECT r = { 0, 0, SCREENW - 1, SCREENH - 1 };
		r2 = r;
	}

	//draw the "partial tile" scroll window onto the backbuffer
